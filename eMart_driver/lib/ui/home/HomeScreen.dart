...
class HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    ...
    // We wrap everything in a PostFrameCallback to ensure the Scaffold and Navigator
    // are ready before we trigger map animations or logic that might show UI.
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (selectedMapType == 'osm') {
        setState(() {
          mapOsmController = osmflutter.MapController(
              initPosition: osmflutter.GeoPoint(latitude: 20.9153, longitude: -100.7439),
              useExternalTracking: false);
        });
      } else {
        // Only animate if the controller has been initialized by onMapCreated
        if (_mapController != null && locationDataFinal != null) {
      ...
      // Load data after the frame is rendered
      getDriver();
      setIcons();
      updateDriverOrder();
    });
  ...
  getCurrentOrder() async {
    ...
    ordersFuture.listen((event) {
      ...
      setState(() {});
      if (selectedMapType == "osm") {
        if (currentOrder?.id != null) getOSMPolyline();
      } else {
        getDirections();
      }
      setState(() {});
        ...
        if (selectedMapType == "osm") {
          setState(() {
            if (osmMarkers.containsKey('Source')) {
              mapOsmController.removeMarker(osmMarkers['Source']!);
            }
  ...
  Widget showDriverBottomSheet() {
    ...
    if (_driverModel!.orderRequestData != null) {
      getDeliveryCharges(
          kilometer, _driverModel!.orderRequestData!.vendorID.toString());
    }
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                Text(
                  // '0',
                  "${kilometer.toStringAsFixed(currencyData?.decimal ?? 2)} km",
                  style: TextStyle(
  ...
  Widget buildOrderActionsCard() {
    ...
    for (var element in currentOrder!.products) {
      if (double.parse(element.discount_price.isEmpty
          ? "0.0"
          : element.discount_price.toString()) <=
          0) {
        subTotal = subTotal +
            double.parse(element.price.toString()) *
                double.parse(element.quantity.toString()) +
            (double.parse(element.extras_price.toString()) *
                double.parse(element.quantity.toString()));
      } else {
        subTotal = subTotal +
    ...
    if (currentOrder!.taxModel != null) {
      for (var element in currentOrder!.taxModel!) {
        taxAmount = taxAmount +
            calculateTax(
                amount:
                (subTotal - double.parse(currentOrder!.discount.toString()))
                    .toString(),
                taxModel: element);
      }
    }

    if (currentOrder!.specialDiscount != null &&
        currentOrder!.specialDiscount!['special_discount'] != null) {
      specialDiscount = double.parse(
          currentOrder!.specialDiscount!['special_discount'].toString());
    }
                          ...
                          body: Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                currentOrder!.payment_method.toLowerCase() ==
                                    "cod"
                                    ? Padding(
  ...
  acceptOrder() async {
    ...
    await SendNotification.sendFcmMessage(
        driverAccepted, orderModel.author.fcmToken, payLoad);
    await SendNotification.sendFcmMessage(
        driverAccepted, orderModel.vendor.fcmToken, payLoad);
    setState(() {
      isShow = true;
    });
  }
  ...
  completeOrder() async {
    ...
    await SendNotification.sendFcmMessage(
        driverCompleted, currentOrder!.author.fcmToken, payLoad);
        ...
        if (currentOrder?.status != ORDER_STATUS_DRIVER_PENDING) {
          if (currentOrder?.status == ORDER_STATUS_SHIPPED) {
            ...
            osmflutter.GeoPoint destinationLocation = osmflutter.GeoPoint(
                latitude: currentOrder?.vendor.latitude ?? 0.0,
                longitude: currentOrder?.vendor.longitude ?? 0.0);
            ...
            roadInfo = await mapOsmController.drawRoad(
              ...
              roadOption: osmflutter.RoadOption(
                ...
                roadBorderWidth: Platform.isIOS ? 15 : 15,
                roadBorderColor: AppThemeData.primary300,
            ...
            mapOsmController.moveTo(
                ...
                animate: true);
            print(
                "sourceLocation :: 00 :: ${sourceLocation.latitude} :: ${sourceLocation.longitude}");
            print(
                "destinationLocation :: 00 :: ${destinationLocation.latitude} :: ${destinationLocation.longitude}");
            print("Location :: 00 :: ${roadInfo.toString()}");
          } else if (currentOrder!.status == ORDER_STATUS_IN_TRANSIT) {
            osmflutter.GeoPoint sourceLocation = osmflutter.GeoPoint(
                latitude: _driverModel?.location.latitude ?? 0.0,
                longitude: _driverModel?.location.longitude ?? 0.0);
            ...
            osmflutter.GeoPoint destinationLocation = osmflutter.GeoPoint(
                latitude: currentOrder?.address.location?.latitude ?? 0.0,
                longitude: currentOrder?.address.location?.longitude ?? 0.0);
            await mapOsmController.removeLastRoad();
            setState(() {});
            roadInfo = await mapOsmController.drawRoad(
              sourceLocation,
            ...
            mapOsmController.moveTo(
                ...
                animate: true);
            print(
                "sourceLocation :: 11 :: ${sourceLocation.latitude} :: ${sourceLocation.longitude}");
            print(
                "destinationLocation :: 11 :: ${destinationLocation.latitude} :: ${destinationLocation.longitude}");
            print("Location :: 11 :: ${roadInfo.toString()}");
          }
        }
      }
    } catch (e) {
