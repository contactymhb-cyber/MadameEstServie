...
String GOOGLE_API_KEY = 'AIzaSyBLCXGFVRHd7BZ7q9ZpAPwywpBjchxDcUc';
...
const GlobalURL = "https://madameestservieguyane.com/";
...
String productCommissionPrice(
    AdminCommissionModel? adminCommision, String price) {
  String commission = "0";
  if (adminCommision != null &&
      adminCommision.enable == true &&
      sectionConstantModel?.adminCommision?.enable == true) {
    if (adminCommision.type?.toLowerCase() == "percent".toLowerCase() ||
        adminCommision.type?.toLowerCase() == "percentage".toLowerCase()) {
      commission = (double.parse(price) +
              (double.parse(price) *
                  double.parse(adminCommision.commission.toString()) /
                  100))
          .toString();
    } else {
      commission = (double.parse(price) +
...
bool isExpire(VendorModel venderModel) {
...
bool isExpireDate(
    {required bool expiryDay, Timestamp? subscriptionExpiryDate}) {
  bool isPlanExpire = false;
  if (expiryDay == true) {
    isPlanExpire = false;
  } else {
