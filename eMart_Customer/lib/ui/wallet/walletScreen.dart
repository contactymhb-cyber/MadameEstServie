...
class WalletScreenState extends State<WalletScreen> {
                                    ...
                                    onTap: () async {
                                      if (!topupTranHistory.isTopup) {
                                        if (topupTranHistory.serviceType == "cab-service") {
                                          await FireStoreUtils.firestore.collection(RIDESORDER).doc(topupTranHistory.order_id).get().then((value) {
                                          ...
                                          await FireStoreUtils.firestore.collection(PARCELORDER).doc(topupTranHistory.order_id).get().then((value) {
                                          ...
                                          await FireStoreUtils.firestore.collection(RENTALORDER).doc(topupTranHistory.order_id).get().then((value) {
                                          ...
                                          print(":::::: value :::::: ${topupTranHistory.order_id}");
                                          await FireStoreUtils.firestore.collection(ORDERS).doc(topupTranHistory.order_id).get().then((value) {
                          ...
                          child: GestureDetector(
                            onTap: () async {
                              if (selectedRadioTile == "Stripe" && stripeData?.isEnabled == true) {
                                ...
                                _flutterWaveInitiatePayment(context, _amountController.text.trim());
  ...
  displayStripePaymentSheet() async {
    ...
    try {
      ...
      showDialog(context: context, builder: (_) => AlertDialog(content: Text("Payment Failed".tr())));
    } catch (e) {
      print('$e');
      Navigator.pop(context);
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
  ...
  payStackPayment() async {
    print("currencyData!.code :: ${currencyData!.code}");
  ...
  _flutterWaveInitiatePayment(BuildContext context, String amount) async {
